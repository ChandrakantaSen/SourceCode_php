...
function getContent($page, $GridOrder = "id"){
	...
	if($_SESSION["GridPage"] > 1){ ?>
		<a href="javascript:getContent('<? echo $_SESSION["GridPage"] - 1 ?>', {target: 'content', preloader: 'listing'});">Previous</a>
	<?PHP }
	...
	$end = ceil($end);
	for($i=$start;$i<$end;$i++){
		if($i != $_SESSION["GridPage"]) $ext = 'href="javascript:' . "getContent('".$i."', {target: 'content', preloader: 'listing'})" . '" style="text-decoration:none;"';
		else $ext = 'style="color:#FF0000; text-decoration:none;"';
		echo '<a' . $ext . '>' . $i . '</a>&nbsp;';
	}
	if($end < ceil($limit)) echo "...";
	if($_SESSION["GridPage"] < ($allCount / $rowsPerPage)){ ?>
		<a href="javascript:getContent('<? echo $_SESSION["GridPage"] + 1 ?>', {target: 'content', preloader: 'listing'});">Next</a>
	<?PHP } ?>
...
$plx = new PHPLiveX(array("getContent"));
...
<tr id="gridHeaders">
<td onclick="getContent('', 'username', {target: 'content', preloader: 'listing'});" onmouseover="GridHeader('over', this)" onmouseout="GridHeader('out', this)" width="150">User Name</td>
<td onclick="getContent('', 'email', {target: 'content', preloader: 'listing'});" onmouseover="GridHeader('over', this)" onmouseout="GridHeader('out', this)" width="190">E-Mail Address</td>
<td onclick="getContent('', 'birthDate', {target: 'content', preloader: 'listing'});" onmouseover="GridHeader('over', this)" onmouseout="GridHeader('out', this)" width="100">Birthdate</td>
<td onclick="getContent('', 'phoneNumber', {target: 'content', preloader: 'listing'});" onmouseover="GridHeader('over', this)" onmouseout="GridHeader('out', this)" width="100">Phone Number</td></tr>
</tbody></table>
<div id="content"></div>

<script language="javascript">
getContent("1", "id", {target: 'content', preloader: 'listing'});
</script>
